...
from src.gan_inference import (
    ...
    load_generator,
    resolve_model_path,
    generate_images_with_manipulated_noise,
    save_figure,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate manipulated vs original comparisons.")
...
def main() -> None:
    ...
    output_path = Path(args.output) if args.output else paths.outputs_dir / "manipulated_vs_original.png"

    generator = load_generator(model_path)
    fig = generate_images_with_manipulated_noise(generator, num_images=args.num_images)
